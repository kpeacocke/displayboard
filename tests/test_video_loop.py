...
def test_check_omxplayer_installed_not_installed() -> None:
    with patch("shutil.which", return_value=None), patch(
        ...
        mock_print.assert_any_call("👉 Install it with: sudo apt install omxplayer")


def test_play_video_loop_keyboard_interrupt() -> None:
    with patch("subprocess.run", side_effect=KeyboardInterrupt), patch(
        "builtins.print"
    ) as mock_print:
        play_video_loop()
...
def test_play_video_loop_called_process_error() -> None:
    # Error then KeyboardInterrupt to exit loop
    error = subprocess.CalledProcessError(1, "omxplayer")
    with patch("subprocess.run", side_effect=[error, KeyboardInterrupt()]), patch(
        "time.sleep"
